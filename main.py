...
# ── DEDUP CACHE — prevents duplicate orders ───────────────────────────────────
...
async def process_order(order: dict):
    """Runs in background after we already responded 200 to Shopify."""
    try:
        ...
        try:
            ...
            purchase_date = f"{dt.month}/{dt.day:02d}/{dt.year} {dt.strftime('%H:%M:%S')}"
        except Exception:
        ...
        append_row(row)
        print(f"✅ Logged order {order_number} - {discount_code}")
...
async def orders_create(request: Request, background_tasks: BackgroundTasks):
    ...
    # Dedup — if we've already seen this order ID, ignore it
    order_id = order.get("id")
    if order_id in _processed_orders:
        print(f"⚠️ Duplicate webhook for order {order_id} — skipping")
        return JSONResponse(content={"status": "duplicate"}, status_code=200)

    # Mark as seen immediately before doing anything else
    _processed_orders.add(order_id)

    # Respond to Shopify instantly — this stops retries
